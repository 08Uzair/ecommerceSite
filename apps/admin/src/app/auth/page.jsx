...
import { createProfile, getProfileByEmail } from "../redux/actions/profile";
...
export default function AuthForm() {
  ...
  const fetchProfile = async (email) => {
    if (email) {
      try {
        ...
        window.localStorage.setItem("adminProfileData", JSON.stringify(data));
      } catch (error) {
  ...
  const handleSignUpChange = (e) => {
    ...
    setProfileData((prev) => ({ ...prev, images, role: "admin" }));
  };
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      fetchProfile(signInData.identifier);
      setLoading(false);
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      router.push("/auth"); // Redirect after sign-in
      fetchProfile(signUpData.email);
      window.location.reload();
    } catch (error) {
      setLoading(false);
      console.log(error);
  ...
  useEffect(() => {
    const profile = window.localStorage.getItem("adminProfileData");
    if (profile) {
